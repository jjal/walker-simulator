...
public:
	...
	void updateWalker(vector<int> state);
	vector<int> getGyroState();
	vector<int> readState();
	...
	vector		<ofxBox2dRect> walkerPolies;
	vector		<ofxBox2dRevoluteJoint> joints;
	ofxBox2dRect bodyLine, lThigh, rThigh, lCalf, rCalf, lFoot, rFoot;
};
